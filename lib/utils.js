...
export function getArcLength(path) {
...
export function bezierArc(angleStart, angleEnd, center, radius) {
  // Finding the coordinates of the control points in a simplified case where the center of the circle is at [0,0]
  const relControlPoints = getRelativeControlPoints(angleStart, angleEnd, radius);

  return {
      pointStart: getPointAtAngle(angleStart, center, radius),
      pointEnd: getPointAtAngle(angleEnd, center, radius),
...
function getRelativeControlPoints(angleStart, angleEnd, radius) {
  ...
  const factor = k(angleStart, angleEnd);
...
function k(angleStart, angleEnd) {
 ...
 * @returns {{Bx: *, By: *}}
 */
export function calculateThirdPoint(Ax, Ay, Cx, Cy, b, c, A, alt) {
